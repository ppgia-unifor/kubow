 ...
 *
 * <p>Records the overall state of the system as system level properties. Mostly used for tracing.
 */
public class ZNNStateAnalyzer extends AbstractRainbowRunnable
    implements IRainbowAnalysis, IModelChangeBusSubscriberPort.IRainbowModelChangeCallback {
  ...
  public static final String MALICIOUS_CLIENTS_SELECTION =
      "/self/components:!PotentiallyMaliciousT[maliciousness>self.MALICIOUS_THRESHOLD]"; // All
                                                                                         // malicious clients
  public static final String ABOVE_MALICIOUSNESS =
      "aboveMaliciousThreshold (self)"; // #malicious clients exceeds threshold
  public static final String HIGH_RT =
      "exists c :! ClientT in self.components | (c.experRespTime > self.MAX_RESPTIME)"; // Is a
                                                                                        // client
                                                                                        // getting
                                                                                        // bad
                                                                                        // response
  private final LinkedBlockingQueue<ZNNModelUpdateOperatorsImpl> m_modelCheckQ =
  ...
  private IModelChangeBusSubscriberPort.IRainbowChangeBusSubscription m_modelChangeSubscriber =
      new ClientResponseTimeGauge.ModelBasedSubscription("ZNewsSys", "Acme") {
        @Override
          ...
          if (type != null) {
            return b
                && ((AcmeModelEventType.ADD_PROPERTY.name().equals(type)
                    ...
                    || (AcmeModelEventType.SET_PROPERTY_VALUE.name().equals(type)
                        && message.getPropertyNames().contains(AcmeModelOperation.PROPERTY_PROP)
                        && ((String) message.getProperty(AcmeModelOperation.PROPERTY_PROP))
                            .endsWith("maliciousness"))
                    || (AcmeModelEventType.SET_PROPERTY_VALUE.name().equals(type)
  ...
  private void initializeAcmeExpressions() {
    ...
    try {
      m_clientsSelection =
          lp.designRuleExpressionFromString(CLIENTS_SELECTION, new RegionManager());
      m_maliciousSelection =
          lp.designRuleExpressionFromString(MALICIOUS_CLIENTS_SELECTION, new RegionManager());
  ...
  protected void runAction() {
    ...
    if (model != null) {
      ...
      if (m_clientsSelection != null && m_maliciousSelection != null) {
        ...
        try {
          AcmeSet clients =
              RuleTypeChecker.evaluateAsSet(system, null, m_clientsSelection, errors, lookup);

          //                    AcmeSet maliciousClients = RuleTypeChecker.evaluateAsSet (system,
          // null, m_maliciousSelection, errors, lookup);
          boolean aboveMal =
              RuleTypeChecker.evaluateAsBoolean(system, null, m_aboveMal, errors, lookup);
          boolean aboveRT =
              RuleTypeChecker.evaluateAsBoolean(system, null, m_aboveRT, errors, lookup);
          float avgRT = 0.0f;
          float perMal = 0.0f;
          if (clients != null) {
            float cum = 0.0f;
            ...
            SetSystemPropertiesCmd cmd =
                model
                    .getCommandFactory()
                    .setSystemProperties(system, avgRT, perMal, aboveRT, aboveMal);
