...
public class AddClientCmd extends ZNNAcmeModelCommand<IAcmeComponent> {
  ...
  /**
   * @param model The model in which the new client may be created
   * @param lb The load balancer that the client will be attached to
   * @param clientIP
   */
  ...
  protected List<IAcmeCommand<?>> doConstructCommand() throws RainbowModelException {
    ...
    if (m_client == null) {
      ...
      IAcmePortCreateCommand httpCmd =
          cf.portCreateCommand(m_clientCmd, "http", HTTP_PORT, HTTP_PORT);
      String connName = ModelHelper.getUniqueName(getModel(), "httpConn");
      IAcmeConnectorCreateCommand httpConnCmd =
          cf.connectorCreateCommand(getModel(), connName, HTTP_CONN, HTTP_CONN);
      ...
      IAcmePortCreateCommand lbPortCmd =
          cf.portCreateCommand(lb, httpPortName, HTTP_PORT, HTTP_PORT);
      IAcmePropertyCreateCommand dlCmd =
          cf.propertyCreateCommand(
              m_clientCmd, "deploymentLocation", "String", new UMStringValue(m_clientIP));
      IAcmeAttachmentCommand att1Cmd =
          cf.attachmentCreateCommand(getModel(), clientName + ".http", connName + ".req");
      IAcmeAttachmentCommand att2Cmd =
          cf.attachmentCreateCommand(getModel(), m_lb + "." + httpPortName, connName + ".rec");
