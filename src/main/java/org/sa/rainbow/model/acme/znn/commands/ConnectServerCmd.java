...
public class ConnectServerCmd extends ZNNAcmeModelCommand<IAcmeComponent> {
  ...
  private static final String DISCONNECTED_QUERY =
      "/self/components:!ServerT[!isArchEnabled and " + "deploymentLocation==\"\"]";
  ...
  public ConnectServerCmd(
      AcmeModelInstance model, String lb, String name, String host, String port) {
  ...
  protected List<IAcmeCommand<?>> doConstructCommand() throws RainbowModelException {
      ...
      if (s_DisconnectedQueryExpr == null) {
        try {
          ...
          throw new RainbowModelException(
              String.format("Could not construct the query expression '%s'", DISCONNECTED_QUERY));
        }
      }
      Stack<AcmeError> errorStack = new Stack<AcmeError>();
      try {
        Object result =
            RuleTypeChecker.evaluateAsAny(
                getModel(), null, s_DisconnectedQueryExpr, errorStack, new NodeScopeLookup());
        if (errorStack.isEmpty() && result instanceof AcmeSet) {
          ...
          if (!set.getValues().isEmpty()) {
            ...
            if (first instanceof IAcmeComponent) srv = (IAcmeComponent) first;
          }
        }
    ...
    if (srv != null) {
      m_server = srv;
      final IAcmePortCreateCommand httpCreateCommand =
          cf.portCreateCommand(srv, "http", ZNNConstants.HTTP_PORT, ZNNConstants.HTTP_PORT);
      String lbName = ModelHelper.getUniqueName(getModel(), "proxyconn");
      // Connector proxyconn : ProxyConnT = new ProxyConnT;
      final IAcmeConnectorCreateCommand proxyConnCreateCmd =
          cf.connectorCreateCommand(
              getModel(), lbName, ZNNConstants.HTTP_CONN_T, ZNNConstants.HTTP_CONN_T);
      String fwd = ModelHelper.getUniqueName(lb, "fwd");
      // m_lb = m_lb extended with {
      //   port fwd : ProxyForwardPortT = new ProxyForwardPortT;
      // }
      final IAcmePortCreateCommand lbPortCmd =
          cf.portCreateCommand(
              lb, fwd, ZNNConstants.PROXY_FORWARD_PORT, ZNNConstants.PROXY_FORWARD_PORT);
      // attachment m_lb.fwd to proxyconn.req;
      final IAcmeAttachmentCommand attachLBEnd =
          cf.attachmentCreateCommand(getModel(), lb.getName() + "." + fwd, lbName + ".req");
      // attachment <m_name>.http to proxyconn.rec;
      final IAcmeAttachmentCommand attachServerEnd =
          cf.attachmentCreateCommand(getModel(), m_name + ".http", lbName + ".rec");

      final IAcmePropertyCommand setLocation =
          cf.propertyCreateCommand(srv, "deploymentLocation", "String", new UMStringValue(m_host));
      final IAcmePropertyCreateCommand setPort =
          cf.propertyCreateCommand(srv, "httpPort", "String", new UMStringValue(m_port));
  ...
  private boolean isConnected(IAcmeComponent srv) {
    ...
    for (IAcmePort port : ports) {
      if (!ModelHelper.getAcmeSystem(srv).getAttachments(port).isEmpty()) return true;
    }
    return false;
  }
