...
public class SetSystemPropertiesCmd extends ZNNAcmeModelCommand<IAcmeSystem> {
  ...
  protected List<IAcmeCommand<?>> doConstructCommand() throws RainbowModelException {
    ...
    if (rtProp == null) {
      cmds.add(
          cf.propertyCreateCommand(
              system, AVERAGE_CLIENT_RESPONSE_TIME, DefaultAcmeModel.defaultFloatType(), rtAcme));
    } else {
    ...
    if (malProp == null) {
      cmds.add(
          cf.propertyCreateCommand(
              system, PERCENTAGE_MALICIOUS, DefaultAcmeModel.defaultFloatType(), malAcme));
    } else {
