package org.sa.rainbow.translator.znn;
...
import static org.sa.rainbow.core.ports.IModelDSBusPublisherPort.Result.UNKNOWN;
      ...
      try {
        ...
        if (object instanceof IAcmeElementInstance) {
          ...
          if (location != null) {
            ...
            Set<EffectorDescription.EffectorAttributes> effectors = getEffectorsAtLocation (location);
            ...
            for (EffectorDescription.EffectorAttributes e : effectors) {
            ...
            if (!effectors.isEmpty ()) {
              ...
              for (EffectorDescription.EffectorAttributes ea : effectors) {
                IEffectorExecutionPort.Outcome outcome = executeEffector (ea.name, ea.getLocation(), cmd.getParameters ());
...
// .result);
  ...
  private Collection<? extends EffectorDescription.EffectorAttributes> getEffectorsInterestedInLocation (String location,
  ...
  private void filterEffectorsBasedOnCommandName ( IRainbowOperation cmd,
    ...
    for (Iterator iterator = effectors.iterator (); iterator.hasNext ();) {
      EffectorDescription.EffectorAttributes ea = (EffectorDescription.EffectorAttributes)iterator.next ();
   ...
   */
  private void filterEffectorsBasedOnCommandParameters (IRainbowOperation cmd, Set<EffectorDescription.EffectorAttributes> effectors, AcmeModelInstance ami) {
    for (Iterator iterator = effectors.iterator (); iterator.hasNext ();) {
      EffectorDescription.EffectorAttributes ea = (EffectorDescription.EffectorAttributes)iterator.next ();
