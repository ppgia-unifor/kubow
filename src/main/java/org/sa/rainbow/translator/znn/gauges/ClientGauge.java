...
public class ClientGauge extends RegularPatternGauge {
  ...
  private static final String[] commandNames = {
    "addClient", "removeClient", "setClientRequestRate"
  };
            ...
            boolean b =
                message.getPropertyNames().contains(IModelChangeBusPort.MODEL_NAME_PROP)
                    && message
                        ...
                        .equals(m_modelDesc.getName());
            if (b) {
              b =
                  message.getPropertyNames().contains(IModelChangeBusPort.EVENT_TYPE_PROP)
                      && message
  ...
  protected void doMatch(String matchName, Matcher m) {
    if (DEFAULT.equals(matchName)) {
      ...
      try {
        ...
        if (m_requestRater != null) m_requestRater.tick(ip);
  ...
  //                        "Client ''{0}'' made {1} requests in the last {2} seconds for a request
  // rate of {3,number,###.######} req/sec",
  //                        client.getQualifiedName (), entry.getValue ().size (), m_clientWindow /
  // 1000, reqRate);
  //                log (format);
