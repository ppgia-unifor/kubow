...
public class ClientResponseTimeGauge extends RegularPatternGauge {
  ...
  public ClientResponseTimeGauge(
      String id,
      long beaconPeriod,
    ...
    m_modelChanges.subscribe(
        new MatchAddClientSubscription(m_modelDesc.getName(), m_modelDesc.getType()),
        new ReportResponseForNewClient());
    m_modelChanges.subscribe(
        new MatchDeleteClientSubscription(m_modelDesc.getName(), m_modelDesc.getType()),
    ...
    if (DEFAULT.equals(matchName)) {
      ...
      if (last == null || new Date().getTime() - last > INTERVAL) {
        ...
        if (history.size() > AVG_SAMPLE_WINDOW) {
          // if queue size reached window size then dequeue and delete olderst value and report
          // average
        ...
        if (client != null) {
          ...
          // Client has not yet been registered with the model. Defer this to a component creation
          // end
        }
  ...
  void issueResponseTimeCommand(
      String client, Queue<Double> history, double cumulation, boolean enqueue) {
    double avgRTms = cumulation / history.size();

    m_reportingPort.trace(
        getComponentType(),
        MessageFormat.format(
            "{0}: {1}, hist{2}", id(), cumulation, Arrays.toString(history.toArray())));
  ...
  public static class ModelBasedSubscription implements IRainbowChangeBusSubscription {
    ...
    public boolean matches(IRainbowMessage message) {
      boolean b =
          message.getPropertyNames().contains(IModelChangeBusPort.MODEL_NAME_PROP)
              && message.getProperty(IModelChangeBusPort.MODEL_NAME_PROP).equals(m_modelName);
      b &=
          message.getPropertyNames().contains(IModelChangeBusPort.MODEL_TYPE_PROP)
  ...
  /**
   * Issues response time reports for new clients. This could happen, for example, when a client is
   * created based on a log entry that has also been read by this gauge.
   */
    ...
    public boolean matches(IRainbowMessage message) {
      ...
      if (type != null) {
        try {
          ...
          b &=
              ct.isEnd()
                  && message.getProperty(IModelChangeBusPort.COMMAND_PROP).equals("deleteClient");
          return b;
        } catch (Exception e) {
        }
      }
