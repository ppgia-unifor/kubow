...
public class RequestRateBasedMaliciousnessGauge extends AbstractGauge {
  ...
  public RequestRateBasedMaliciousnessGauge(
      String id,
      long beaconPeriod,
    ...
    m_modelChanges.subscribe(
        new RequestRatePropertySubscription(modelDesc.getName(), modelDesc.getType()),
  ...
  public class ProcessRequestRate implements IRainbowModelChangeCallback {
    ...
    public void onEvent(ModelReference reference, IRainbowMessage message) {
      if (!isTerminated()) {
        ...
        String reqRateString =
            (String) message.getProperty(IModelChangeBusPort.PARAMETER_PROP + "0");
